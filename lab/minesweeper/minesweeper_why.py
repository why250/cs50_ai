...
class Sentence():
    ...
    def __init__(self, cells, count): #what to do when Sentence is created
        self.cells = set(cells)
        self.count = count

    def __eq__(self, other):# what to do when two sentences are compared
        return self.cells == other.cells and self.count == other.count

    def __str__(self):# what to do when print() is called on a sentence
...
class MinesweeperAI():
    ...
    def conclude(self):
        ...
        for sentence in self.knowledge:
            if sentence.known_mines():
                new_clue += 1
                mine_cells += list(sentence.known_mines())#convert set to list
    ...
    def add_knowledge(self, cell, count):
        ...
        """
        self.moves_made.add(cell)#add 用于集合，确保元素唯一
        self.safes.add(cell)
        neighbour_cells = set()
        for i in range(cell[0]-1,cell[0]+2):
            for j in range(cell[1]-1,cell[1]+2):
        ...
        while 1:
            new_clue_c = self.conclude()
            new_clue_i = self.infer()
            if new_clue_c == new_clue_i == 0:
                break
    ...
    def make_safe_move(self):
        ...
        return random.choice(safe_moves) if len(safe_moves) != 0 else None
